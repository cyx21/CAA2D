...
clear all
clc
data.Nx=200;data.Ny=100;%100列，100行
data.dt=0.2;data.c=1.0;data.r0=1.0;data.gamma=0.2*pi;
data.a=50*data.r0;
data.path='E:\jet\jet_gradmesh_80sec_upwind\';
data.top=10;data.bottom=-10;data.left=-20;data.right=20;
...
data.Nt=100;%floor(2*data.a/(data.c*data.dt))*2;
CFL=data.c*data.dt/data.dx;
...
% shading interp
[q]=initial(data);
surf(data.xx,data.yy,q(:,:,4)*data.c^2)
shading interp
...
for tstep=20:1:data.Nt
  [newq]=LDDRK(q,data,tstep);
    % [newq]=RK4(q,data,tstep);
     %[newq]=RK1(q,data,tstep);
    q=newq;
maxU=max(max(q(:,:,2)));
maxV=max(max(q(:,:,3)));
CFL=sqrt(maxU^2+maxV^2)*data.dt/data.dx;
    figure(1)
    surf(data.xx,data.yy,q(:,:,2))
    shading interp
...
%     'Neutral','Warm','Hot','Burning','Nuclear'})
    set(gca,'CLim',[a b])
    title(['tstep=',num2str(tstep),'  CFL=', num2str(CFL)])
    view(0,90)
    xlabel('X')
    ylabel('Y')
    pause(0.1)
...
function [newq]=RK1(q,data,tstep)
...
function [newq]=RK4(q,data,tstep)
...
[dqdx,dqdy]=compact10order(dqdx,dqdy,q,data);
%[dqdx,dqdy]=center4orderall(q,data,dqdx,dqdy);
%[dqdx,dqdy]=drp7all(q,data,dqdx,dqdy);
%[dqdx,dqdy]=drp7_comp6bound(q,data);
[U ,V ,P ,P1,P2]=buildinc_jet(data,tstep);
[S]=buildS_jet(q,data,tstep,U ,V ,P ,P1,P2);

 for i=1:1:data.Ny
    for j=1:1:data.Nx
        %[Uij,Vij,Pij]=buildinc(data,i,j,tstep);
        rho0=1.0;
        u=U(i,j)+q(i,j,2);v=V(i,j)+q(i,j,3);rho=1+q(i,j,1);p=P(i,j)+q(i,j,4)+1*data.c^2;
        ratio=1;%c=(1.4*(abs(Pij+q(i,j,4)))/rho)^0.5;
        if (i>boundi&&i<data.Ny-boundi+1)&&(j>boundi&&j<data.Nx-boundi+1)
           

%             
%             [tao_1,tao_2]=viscoustensor(i,j,dqdx,dqdy,data);
%             S(2)=S(2)+tao_1;
%             S(3)=S(3)+tao_2;
...
%           A=[u rho 0 0;0 u 0 1.0/rho;0  0 u 0;u*data.c^2 rho*data.c^2 0 0;];
%           B=[v 0 rho 0;0 v 0 0;0 0 v 1.0/rho;v*data.c^2 0 rho*data.c^2 0;];
%               A=[U(i,j) rho0 0 0;0 U(i,j) 0 1.0/rho0;0 0 U(i,j) 0;0 ratio*P(i,j) 0 U(i,j);];
%              B=[V(i,j) 0 rho0 0;0 V(i,j) 0 0;0 0 V(i,j) 1.0/rho0;0 0 ratio*P(i,j) V(i,j);];
            %test%
%             A=[Uij rho0 0 0;0 Uij Vij 1.0/rho0;0 0 0 0;0 ratio*Pij 0 Uij;];
%             B=[Vij 0 rho0 0;0 0 0 0;0 Uij Vij 1.0/rho0;0 0 ratio*Pij Vij;];
            %%%%%%
            pqpx=[dqdx(i,j,1);dqdx(i,j,2);dqdx(i,j,3);dqdx(i,j,data.var)];
            pqpy=[dqdy(i,j,1);dqdy(i,j,2);dqdy(i,j,3);dqdy(i,j,data.var)];
            QQ=-A*pqpx-B*pqpy;
            
            Q(i,j,1)= S(i,j,1)+QQ(1);
            Q(i,j,2)= S(i,j,2)+QQ(2);
             Q(i,j,3)= S(i,j,3)+QQ(3);
              Q(i,j,4)= S(i,j,4)+QQ(4);
        elseif j>=data.Nx-boundi+1
             r=(data.x(j)^2+data.y(i)^2)^0.5;
          cos =data.x(j)/r;sin =data.y(i)/r;
           vel=data.c+U(i,j)*cos+V(i,j)*sin;
           %辐射边界条件
           Q(i,j,:)=vel*(-cos*dqdx(i,j,:)-sin*dqdy(i,j,:)-q(i,j,:)/(2*r));
           %出流边界条件
%            coswave=(data.x(j)*Uij+data.y(i)*Vij)/((Uij^2+Vij^2)^0.5*(data.x(j)^2+data.y(i)^2)^0.5);
%             cosr=data.x(j)/r;sinr=data.y(i)/r;
%            Vr=(Uij^2+Vij^2)^0.5*cos(coswave)+(data.c^2-(Uij^2+Vij^2)*(1-coswave^2))^0.5;
%           %Vr=(Uij^2+Vij^2)^0.5+data.c;
%            Q(i,j,1)=(-Vr*(cosr*dqdx(i,j,4)+sinr*dqdy(i,j,4))-Vr/(2*r)*q(i,j,4))/data.c^2+Uij/data.c^2*dqdx(i,j,4)+Vij/data.c^2*dqdy(i,j,4)-Uij*dqdx(i,j,1)-Vij*dqdy(i,j,1);
%            Q(i,j,2)=-Uij*dqdx(i,j,2)-Vij*dqdy(i,j,2)-1/rho0*dqdx(i,j,4);
%            Q(i,j,3)=-Uij*dqdx(i,j,3)-Vij*dqdy(i,j,3)-1/rho0*dqdy(i,j,4);
%            Q(i,j,4)=(-Vr*(cosr*dqdx(i,j,4)+sinr*dqdy(i,j,4))-Vr/(2*r)*q(i,j,4));
...
%             cosr=data.x(j)/r;sinr=data.y(i)/r;
           ...
           % vel=data.c+u*cos+v*sin;
           %辐射边界条件
          % Q(i,j,:)=Vr*(-cosr*dqdx(i,j,:)-sinr*dqdy(i,j,:)-q(i,j,:)/(2*r));
...
%     S4=-dPdt-ratio*(q(i,j,2)+q(i,j,1)/rho*Uij)*dPdx-ratio*(q(i,j,3)+q(i,j,1)/rho*Vij)*dPdy;
    S2=-q(i,j,2)*dUdx-q(i,j,3)*dVdx;
    S3=-q(i,j,2)*dUdy-q(i,j,3)*dVdy;  
%     S4=-dPdt-u*dPdx-v*dPdy;
    % S=[0;0;0;S4;];
    %S4=0.0;
   S=[0;S2;S3;S4;];
...
function [S]=buildS_jet(q,data,tstep,U0,V0,P0,P1,P2)
   
   S=zeros(data.Ny,data.Nx,4);
    if tstep==1
        dPdt=zeros(data.Ny,data.Nx) ;
    elseif tstep>1 && tstep<data.Nt
       
       dPdt=(1/(2*data.dt))*(P2-P1);
    elseif tstep ==data.Nt
        dPdt=1/(data.dt)*(P2-P1);
    end
    ...
    for i=2:1:data.Ny-1
        for j=2:1:data.Nx-1
        dUdy=(U0(i+1,j)-U0(i-1,j))/(2*data.dy);
        dVdy=(V0(i+1,j)-V0(i-1,j))/(2*data.dy);
        dPdy=(P0(i+1,j)-P0(i-1,j))/(2*data.dy);
        dUdx=(U0(i,j+1)-U0(i,j-1))/(2*data.dx);
        dVdx=(V0(i,j+1)-V0(i,j-1))/(2*data.dx);
        dPdx=(P0(i,j+1)-P0(i,j-1))/(2*data.dx);
        S4=-dPdt(i,j)-u(i,j)*dPdx-v(i,j)*dPdy;%;
        S2=-q(i,j,2)*dUdx-q(i,j,3)*dVdx;
        S3=-q(i,j,2)*dUdy-q(i,j,3)*dVdy;  
        S(i,j,:)=[0;S2;S3;S4;];
        %S(i,j,:)=[0;0;0;S4;];
...
qghosttop=0.5*(q(data.Ny,:,:)+q(data.Ny-1,:,:));
...
elseif (i==5&&j>4&&j<data.Nx-3)||(i==data.Ny-4&&j>4&&j<data.Nx-3)||(j==5&&i>4&&i<data.Ny-3)||(j==data.Nx-4&&i>4&&i<data.Ny-3)
    ...
    qtmp(i,j,:)=q(i,j,:)-0.5*Dfx-0.5*Dfy;
elseif (i==4&&j>3&&j<data.Nx-2)||(i==data.Ny-3&&j>3&&j<data.Nx-2)||(j==4&&i>3&&i<data.Ny-2)||(j==data.Nx-3&&i>3&&i<data.Ny-2)
    ...
    qtmp(i,j,:)=q(i,j,:)-0.5*Dfx-0.5*Dfy;
elseif (i==3&&j>2&&j<data.Nx-1)||(i==data.Ny-2&&j>2&&j<data.Nx-1)||(j==3&&i>2&&i<data.Ny-1)||(j==data.Nx-2&&i>2&&i<data.Ny-1)
    ...
    qtmp(i,j,:)=q(i,j,:)-0.5*Dfx-0.5*Dfy;
elseif (i==2&&j>1&&j<data.Nx)||(i==data.Ny-1&&j>1&&j<data.Nx)||(j==2&&i>1&&i<data.Ny)||(j==data.Nx-1&&i>1&&i<data.Ny)
...
%         %q(i,j,data.var)=exp(-0.693147*(data.x(j)-data.y(i))^2/9);
%         q(i,j,1)=q(i,j,data.var)/data.c^2;
%         
%     end
% end
end
function [U,V,P,P1,P2]=buildinc_jet(data,tstep)
time=tstep*data.dt;
if tstep>1&&tstep<data.Nt
	path_P1=[data.path num2str(time-data.dt) '\coarse_Pij.dat'];
    path_P2=[data.path num2str(time+data.dt) '\coarse_Pij.dat'];
elseif tstep == data.Nt
    path_P1=[data.path num2str(time-data.dt) '\coarse_Pij.dat'];
    path_P2=[data.path num2str(time) '\coarse_Pij.dat'];
elseif tstep==1
     path_P1=[data.path num2str(time) '\coarse_Pij.dat'];
    path_P2=path_P1;
end
    Ujet=importdata([data.path num2str(time) '/coarse_Uij.dat']) ;
    Vjet=importdata([data.path num2str(time) '/coarse_Vij.dat']);
    Pjet=importdata([data.path num2str(time) '/coarse_Pij.dat']);
    P1jet=importdata(path_P1);
    P2jet=importdata(path_P2);
    U=zeros(data.Ny,data.Nx);
    V=zeros(data.Ny,data.Nx);
    P=zeros(data.Ny,data.Nx);
    P1=zeros(data.Ny,data.Nx);
    P2=zeros(data.Ny,data.Nx);
    Nyinctop=data.Ny-floor(abs(data.top-data.inctop)/data.dy);
    Nyincbot=floor(abs(data.bottom-data.incbottom)/data.dy)+1;
    Nxincleft=floor(abs(data.left-data.incleft)/data.dx)+1;
    Nxincright=data.Nx-round( abs(data.right-data.incright)/data.dx);
    U(Nyincbot:Nyinctop,Nxincleft:Nxincright)=Ujet;
    V(Nyincbot:Nyinctop,Nxincleft:Nxincright)=Vjet;
    P(Nyincbot:Nyinctop,Nxincleft:Nxincright)=Pjet;
    P1(Nyincbot:Nyinctop,Nxincleft:Nxincright)=P1jet;
    P2(Nyincbot:Nyinctop,Nxincleft:Nxincright)=P2jet;
end
...
% if (data.y(i)^2+data.x(j)^2<=(0.5*data.a)^2)
% %     
    %%%vortex pair%%%
...
function [anglewave]=angle_calcu(i,j,data,Uij,Vij)
anglewave=atan(data.y(i)/data.x(j))-atan(Vij/Uij);
end
